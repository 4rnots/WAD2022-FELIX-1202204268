...
@section('section')
  ...
  <div class="container mt-4">
    ...
    <div class="row mt-5 justify-content-between">
      <div class="col-md-5">
        <img class="rounded-3" src="{{ asset('storage/image') }}/{{ $showroom->image }}" alt="{{ $showroom->image }}" style="width: 95%" height="400px">
      </div>
      <div class="col-md-7">
        <form action="/showrooms/{{ $showroom->id }}" method="post" enctype="multipart/form-data">
          @csrf
          @method('put')
          <div class="mb-3">
          ...
          <div class="mb-3">
            ...
            <input type="text" name="owner" class="form-control" id="owner" value="{{ $showroom->owner }}">
          </div>
          <div class="mb-3">
            <label for="brand" class="form-label fw-bold">Merk</label>
            <input type="text" name="brand" class="form-control" id="brand" value="{{ $showroom->brand }}">
          </div>
          <div class="mb-3">
            <label for="purchase_date" class="form-label fw-bold">Tanggal Beli</label>
            <input type="date" name="purchase_date" class="form-control" id="purchase_date" value="{{ $showroom->purchase_date }}">
          </div>
          <div class="mb-3">
            <label for="description" class="form-label fw-bold">Deskripsi</label>
            <textarea name="description" class="form-control" id="description" rows="4">{{ $showroom->description }}</textarea>
          </div>
          ...
          <div class="mb-3">
            ...
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" name="status" id="belum-lunas" value="Belum Lunas" {{ $showroom->status === "Belum Lunas" ? "checked" : "" }}>
